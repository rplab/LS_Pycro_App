...
from LS_Pycro_App.acquisition.sequences.imaging import Snap, Video, SpectralVideo, ZStack, SpectralZStack
...
class AcquisitionOrder(ABC):
    ...
    def _acquire_snap(self, region: Region):
        ...
        # for more details.
        snap = Snap(region, self._acq_settings, self._abort_flag, self._acq_directory)
    ...
    def _acquire_video(self, region: Region):
        for attempt_num in range(self.ATTEMPT_LIMIT):
            try:
                video = Video(region, self._acq_settings, self._abort_flag, self._acq_directory)
    ...
    def _acquire_spectral_video(self, region: Region):
        spectral_video = SpectralVideo(region, self._acq_settings, self._abort_flag, self._acq_directory)
    ...
    def _acquire_z_stack(self, region: Region):
        for attempt_num in range(self.ATTEMPT_LIMIT):
            try:
                z_stack = ZStack(region, self._acq_settings, self._abort_flag, self._acq_directory)
    ...
    def _acquire_spectral_z_stack(self, region: Region):
        spectral_z_stack = SpectralZStack(region, self._acq_settings, self._abort_flag, self._acq_directory)
...
class PositionTimeAcquisition(AcquisitionOrder):
    def run(self):
        if not self._get_start_region(0)[0]:
            raise exceptions.AbortAcquisitionException("No valid region for imaging")
